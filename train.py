...
data_labels_paths = {(k*2)+1: f"data/new_synthetic/{k}.txt" for k in range(1, 7)}
...
dataset_sizes = {(k*2)+1: [6, 6] for k in range(1, 7)}
...
def one_hot_labels(labels):
    ...
    oh_labels = torch.cat((
        ...
        F.one_hot(torch.from_numpy(labels_type), 3)
    ), dim=2).float()
    ...
    for batch_idx in range(config.test_size // config.batch_size):
        for k in data_labels_paths.keys():
            with torch.no_grad():
                ...
                data = Variable(torch.from_numpy(data_[:, :, 0:config.top_k + 1, :, :, :])).cuda()
                data = data.permute(1, 0, 2, 3, 4, 5)

                test_output = imitate_net([data, oh_labels, k])

                l = imitate_net.loss_function(test_output, labels)
                test_losses += l
